...
// ═══════════════════════════════════════════════════════════════════════════════
//  @AI_GUARD: CENTRAL_SCHEMA - 通用内部消息模型，所有协议翻译的中枢，绝对不可随意修改
//  @CONSTRAINT: 修改任何字段名、类型、语义必须同步修改所有 4 个协议翻译器
//    - 字段增删：必须检查 anthropic/chatcompletion/gemini/responses 四个 translator.go
//    - 字段语义变更：必须更新所有 TranslateRequest + TranslateResponse + TranslateStream
//    - 新增结构体：必须添加对应的协议差异注释，说明各协议如何映射
//  @RELATED: internal/protocol/*/translator.go, internal/translator/interfaces.go
//  @REASON: 历史血泪教训 - 修改 InternalContentBlock 字段导致图片传输丢失，修改 InternalTool
//    导致 tool_calls 参数格式错误，修改 Stream 字段导致流/非流路由混乱
//
...
// @AI_GUARD: INTERNAL_MESSAGE - 中枢消息对象，所有协议消息转换的核心数据结构
// @CONSTRAINT: 修改字段必须同步所有翻译器的消息转换逻辑
//   - Role: 各协议角色名不同，翻译器负责映射
//   - Content: json.RawMessage 保留原始格式，不可改为具体类型
//   - ContentBlocks: 多模态内容块，出站优先使用此字段还原图片
//   - ToolCalls: tool 调用，各协议字段路径不同
//
// @RELATED: all protocol/translator.go messageToInternal/internalMessageTo* 函数
//
...
// @CONSTRAINT: 字段增删必须同步所有翻译器的内容块解析/构建逻辑
//   - Thinking/Signature: Anthropic 扩展思考字段，不可随意删除
//   - Data/MediaType: 图片 base64 数据，各协议格式不同但最终汇聚于此
//   - RawExtension: 协议特有字段兜底，不可删除（防止信息丢失）
//
// @RELATED: all protocol/translator.go content block parse/build 函数
//
// ─── 内容块 (多模态) ─────────────────────────────────────────────────
//
// 各协议内容块差异：
...
// @AI_GUARD: INTERNAL_TOOL - 工具定义，各协议 tool/function 格式差异的汇聚点
// @CONSTRAINT: 字段修改必须同步所有翻译器的 tool 翻译逻辑
//   - Function.Parameters: CC 格式，其他协议翻译时需映射
//   - InputSchema: Anthropic 格式，与 Parameters 二选一
//
// @RELATED: all protocol/translator.go tool 翻译函数
//
...
// @AI_GUARD: INTERNAL_REQUEST - 中枢请求对象，所有协议入站翻译的目标结构
...
// @AI_GUARD: INTERNAL_RESPONSE - 中枢响应对象，所有协议出站翻译的源结构
...
// @CONSTRAINT: 字段增删/语义变更必须同步所有协议翻译器的 TranslateStream + TranslateStreamEvent
//   - Type: "start"/"delta"/"tool_call"/"done"/"error"/"usage"，各协议事件映射不同
//   - Data.Choices[].Delta: 流式增量，各协议 delta 路径不同
//   - Error: 流式错误，需统一格式
//
// @RELATED: all protocol/translator.go TranslateStream/TranslateStreamEvent
//
// ─── Streaming ──────────────────────────────────────────────────────
//
// SSE 事件差异（最复杂的兼容点）：
